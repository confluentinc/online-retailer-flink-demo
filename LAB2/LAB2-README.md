...
3. Click on the **Tags** tab
   ...
   ![Create catalog tags](./assets/LAB2_catalog_create_tags.png)

4. Click on **Create tags**
5. Select **PII** from the recommended tags

   ![Recommended tag list](./assets/LAB2_catalog_pii.png)
...
1. In the `payments` topic, notice that the topic contains unencrypted credit card information.
   ...
   ![Topic message details showing fake unencrypted CC](./assets/LAB2_cc_unencrypted.png)
...
3. Check if there are any duplicates in `payments` table by executing this query:
   ...
   ![List of duplicate payments](./assets/LAB2_flink_duplicate_payments.png)
...
1. Navigate to the `completed_orders` topic
2. Click on **Enable Tableflow**
3. Click **Use Confluent storage**
...
<details>
   ...
   As an alternative to using Confluent Managed Storage, you can leverage Amazon S3 storage in your own Amazon S3 Bucket.
   ...
   4. In the next menu, you will be able to choose the Provider Integration we created in the previous section. You can identify it by either the name of the provider integration or the IAM Role you created.

   5. There should be only one provider integration created, so select that one. It can also be found in the outputs by running `terraform output resource-ids` and looking for the provider integration name.

   6. Provide the AWS S3 bucket name (`shiftleft-tableflow-bucket...`) which can be found in the `resource-ids` list from terraform as well.

      ![instructions for self storage](assets/byos-instructions.png)

   7. Click Continue and Launch. The tableflow topic will now be materialized in the Amazon S3 Bucket you specified.
